...
class TestFileStorage(unittest.TestCase):
    ...
    def test_reload(self):
        ...
        self.storage.new(base_model)
        self.storage.save()
        with open("AirBnB_storage.json", "r") as file:
            text = file.read()
    ...
    def test_multiple_instances(self):
        ...
        self.assertEqual(
            self.storage.all()[
                f"{BaseModel.__name__}.{base_model.id}"].to_dict(),
            base_model.to_dict(),
        )
        self.assertEqual(
        ...
        self.assertEqual(
            self.storage.all()[
                f"{Amenity.__name__}.{amenity_model.id}"].to_dict(),
            amenity_model.to_dict(),
        )
        self.assertEqual(
            self.storage.all()[
                f"{Review.__name__}.{review_model.id}"].to_dict(),
